...
import { Category, CreateArticleInput } from '@/types';
...
export default function NewArticle() {
  ...
  const handleAutoSave = async (data: CreateArticleInput) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (result.success && result.data?.id && !draftId) {
          setDraftId(result.data.id); // Store the draft ID for future auto-saves
        }
  ...
  const handleWordCountChange = (words: number, chars: number, readTime: number) => {
    ...
    // Update the form data with calculated reading time
    setFormData(prev => ({
      ...prev,
      // We could store reading time in the form data if needed
    }));
  };
  ...
  const { save: manualSave } = useAutoSave({
    ...
    enabled: formData.title.trim().length > 0,
  });
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4 text-sm text-gray-500">
            ...
            <div className="flex items-center space-x-2">
              ...
              {draftId && (
                <span className="text-xs bg-gray-100 px-2 py-1 rounded">Draft ID: {draftId.slice(-6)}</span>
              )}
            </div>
          </div>
        </div>
              ...
              <div className="border border-gray-200 rounded-md p-4 bg-gray-50">
                ...
                <div className="space-y-1">
                  ...
                  <div className="text-green-700 text-sm truncate">
                    yoursite.com/articles/{formData.title ? slugify(formData.title) : 'article-slug'}
                  </div>
